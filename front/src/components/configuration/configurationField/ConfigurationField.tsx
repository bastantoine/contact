...
function ConfigurationField(props: PropsType & {children?: React.ReactNode}) {
    ...
    const [isButtonFormHelpTextDisplayed, setIsButtonFormHelpTextDisplayed] =
        useState(
            props.fieldConfig.form_help_text !== undefined ||
                props.errorsFields.includes(`${props.fieldKey}-form_help_text`)
        );
    const [isButtonMainAttributeDisplayed, setIsButtonMainAttributeDisplayed] =
        useState(
    ...
    const [displayNameHook, setDisplayNameHook] = useState(
        props.fieldConfig.display_name
    );
    ...
    const [selectAllowedValuesHook, setSelectAllowedValuesHook] = useState<
        [string, string][]
    >(
        props.fieldConfig.additional_type_parameters &&
            props.fieldConfig.additional_type_parameters.allowed_values
            ? props.fieldConfig.additional_type_parameters.allowed_values.map(
                  (val: string) => [val, val]
              )
            : [[getRandomString(), ""]]
    );
    ...
    const [deletedSelectAllowedValuesHook, setDeletedSelectAllowedValuesHook] =
        useState<string[]>([]);

    return (
        <>
            <div
                className={`field-configuration-form ${
                    props.errorsFields.length > 0
                        ? "field-configuration-form-with-errors"
                        : ""
                }`}
            >
                <div
                ...
                >
                    <span className="field-configuration-name-title-value">
                        {getFieldTitle(fieldNameHook, displayNameHook)}
                    </span>
                </div>
                <Collapse in={open}>
                    <div id={`config-form-field-${props.fieldName}`}>
                        <ConfigurationFieldElement.Name
                            ...
                            onChange={(event: _EventType) => {
                                setFieldNameHook(event.target.value);
                                props.onChange(event);
                            }}
                            onBlur={props.onBlur}
                            isInvalid={props.errorsFields.includes(
                                `${props.fieldKey}-name`
                            )}
                        />
                        <ConfigurationFieldElement.Type
                            ...
                            defaultValue={
                                Object.keys(
                                    ATTRIBUTE_TYPE_COMPONENT_MAPPING
                                ).includes(String(props.fieldConfig.type))
                                    ? props.fieldConfig.type
                                    : "Choose type..."
                            }
                            onChange={(event: _EventType) => {
                                setTypeHook(event.target.value);
                                props.onChange(event);
                            }}
                            onBlur={props.onBlur}
                            isInvalid={props.errorsFields.includes(
                        ...
                        >
                            ...
                            {Object.keys(ATTRIBUTE_TYPE_COMPONENT_MAPPING).map(
                                (type) => (
                            ...
                            <Col xl={9}>
                                <Form.Row>
                                    <Form.Check
                                        ...
                                        label="Required"
                                        defaultChecked={
                                            props.fieldConfig.required === true
                                                ? true
                                                : undefined
                                        }
                        ...
                        {typeHook === "list" ? (
                            <ConfigurationFieldElement.InnerType
                                ...
                                defaultValue={
                                    props.fieldConfig.additional_type_parameters
                                        ? LIST_ALLOWED_INNER_TYPES.includes(
                                              props.fieldConfig
                                ...
                                onBlur={props.onBlur}
                                isInvalid={props.errorsFields.includes(
                                    `${props.fieldKey}-inner_type`
                                )}
                            >
                                <option>Choose type...</option>
                                {LIST_ALLOWED_INNER_TYPES.map((type) => (
                                    <option key={type}>{type}</option>
                                ))}
                            </ConfigurationFieldElement.InnerType>
                        ) : (
                        ...
                        {typeHook === "image" ? (
                            <ConfigurationFieldElement.AcceptedTypes
                                fieldKey={props.fieldKey}
                                fieldName={props.fieldName}
                            ...
                            <Form.Group as={Row}>
                                ...
                                <Col xl={9}>
                                    <Row>
                                        <Col
                                            xl={6}
                                            className="toggle-field-display-value-true"
                                        >
                                            <Form.Control
                                                type="text"
                                                placeholder="True"
                                        ...
                                        <Col
                                            xl={6}
                                            className="toggle-field-display-value-false"
                                        >
                                            <Form.Control
                                                type="text"
                                                placeholder="False"
                        ...
                        {typeHook === "select" ? (
                            <Form.Group as={Row}>
                                ...
                                <Col xl={9}>
                                        ...
                                        ([key, value]: [string, string]) => {
                                            ...
                                            return (
                                                <InputGroup
                                                    ...
                                                    key={key}
                                                    hidden={
                                                        deletedSelectAllowedValuesHook.includes(
                                                            key
                                                        )
                                                            ? true
                                                            : undefined
                                                ...
                                                >
                                                    ...
                                                    <InputGroup.Append>
                                                        <Button
                                                            variant="danger"
                                                            onClick={() => {
                                                                setDeletedSelectAllowedValuesHook(
                                                                    [
                                                                        ...deletedSelectAllowedValuesHook,
                                                                        key,
                                                                    ]
                                                                );
                                                                props.setFieldValue(
                                    ...
                                    <Form.Text muted>
                                        Enter the values that will be available
                                        in the select.
                                    </Form.Text>
                                    <Row>
                                        <Col xl={{offset: 8, span: 4}}>
                                            <Button
                                                ...
                                                onClick={() =>
                                                    setSelectAllowedValuesHook([
                                                        ...selectAllowedValuesHook,
                                                        [getRandomString(), ""],
                                                    ])
                                                }
                                            >
                        ...
                        <Row>
                            <Col xl={12} style={{marginBottom: "16px"}}>
                                ...
                                >
                                    <Button
                                        variant={
                                            isButtonDisplayedNameDisplayed
                                                ? "outline-success"
                                                : "outline-secondary"
                                        }
                                        className="w-100"
                                        onClick={() =>
                                            setIsButtonDisplayedNameDisplayed(
                                                !isButtonDisplayedNameDisplayed
                                            )
                                        }
                                    >
                                    ...
                                    <Button
                                        variant={
                                            isButtonFormHelpTextDisplayed
                                                ? "outline-success"
                                                : "outline-secondary"
                                        }
                                        className="w-100"
                                        onClick={() =>
                                            setIsButtonFormHelpTextDisplayed(
                                                !isButtonFormHelpTextDisplayed
                                            )
                                        }
                                    >
                                    ...
                                    <Button
                                        variant={
                                            isButtonMainAttributeDisplayed
                                                ? "outline-success"
                                                : "outline-secondary"
                                        }
                                        className="w-100"
                                        onClick={() =>
                                            setIsButtonMainAttributeDisplayed(
                                                !isButtonMainAttributeDisplayed
                                            )
                                        }
                                    >
                                    ...
                                    <Button
                                        variant={
                                            isButtonSortKeyDisplated
                                                ? "outline-success"
                                                : "outline-secondary"
                                        }
                                        className="w-100"
                                        onClick={() =>
                                            setIsButtonSortKeyDisplated(
                                                !isButtonSortKeyDisplated
                                            )
                                        }
                                    >
                        ...
                        {isButtonDisplayedNameDisplayed ? (
                            <ConfigurationFieldElement.DisplayName
                                fieldKey={props.fieldKey}
                                fieldName={props.fieldName}
                                defaultValue={props.fieldConfig.display_name}
                                onBlur={props.onBlur}
                                onChange={(event) => {
                                    setDisplayNameHook(event.target.value);
                                    props.onChange(event);
                                }}
                                isInvalid={props.errorsFields.includes(
                                    `${props.fieldKey}-display_name`
                                )}
                            />
                        ) : (
                            <></>
                        ...
                        {isButtonFormHelpTextDisplayed ? (
                            <ConfigurationFieldElement.FormHelpText
                                fieldKey={props.fieldKey}
                                fieldName={props.fieldName}
                        ...
                        {isButtonMainAttributeDisplayed ? (
                            <ConfigurationFieldElement.MainAttribute
                                fieldKey={props.fieldKey}
                                fieldName={props.fieldName}
                        ...
                        {isButtonSortKeyDisplated ? (
                            <ConfigurationFieldElement.SortKey
                                fieldKey={props.fieldKey}
                                fieldName={props.fieldName}
                        ...
                        <Row>
                            <Col xl={{offset: 9, span: 3}}>
                                <Button
                                    ...
                                    className="button-full-width"
                                    onClick={() =>
                                        props.deleteFieldHandler(props.fieldKey)
                                    }
                                >
                                    Delete the field
                                </Button>
