                    ...
                    onSubmit={(
                        ...
                        for (let fieldKey of this.state.fields) {
                            ...
                            for (let param of Object.keys(fieldValues)) {
                                if (
                                    ...
                                    fieldValues[param] = Array.isArray(
                                        fieldValues[param]
                                    )
                                        ? fieldValues[param].length > 0
                                        : fieldValues[param];
                            }
                            if (
                                fieldValues.type === "list" &&
                                fieldValues.inner_type
                            ) {
                                fieldValues.additional_type_parameters = {
                                    inner_type: fieldValues.inner_type,
                                };
                                delete fieldValues.inner_type;
                            }
                            if (
                                fieldValues.type === "image" &&
                                fieldValues.accepted_types
                            ) {
                                fieldValues.additional_type_parameters = {
                                    accepted_types: fieldValues.accepted_types
                                        .split(",")
                                        .map((v: string) => v.trim()),
                                };
                            ...
                            if (
                                fieldValues.type === "toggle" &&
                                (fieldValues.value_true !== undefined ||
                                    fieldValues.value_false !== undefined)
                            ) {
                                ...
                                fieldValues.additional_type_parameters = {
                                    value_true:
                                        fieldValues.value_true !== undefined
                                            ? fieldValues.value_true
                                            : fieldValues
                                                  .additional_type_parameters
                                                  .value_true || "",
                                    value_false:
                                        fieldValues.value_false !== undefined
                                            ? fieldValues.value_false
                                            : fieldValues
                                                  .additional_type_parameters
                                                  .value_false || "",
                                };
                                fieldValues.value_true !== undefined &&
                                    delete fieldValues.value_true;
                                fieldValues.value_false !== undefined &&
                                    delete fieldValues.value_false;
                            }
                            if (fieldValues.type === "select") {
                                ...
                                if (!fieldValues.additional_type_parameters)
                                    fieldValues.additional_type_parameters = {};
                                if (
                                    fieldValues.additional_type_parameters
                                    ...
                                    fieldValues.additional_type_parameters.allowed_values =
                                        fieldValues.additional_type_parameters.allowed_values.filter(
                                            (value: string) =>
                        ...
                        fetchOrThrow(join(API_ENDPOINT, "config"), {
                            ...
                            .catch(async (error: APIError) => {
                                const error_json_body =
                                    await error.response.json();
                                this.setState({
                                    submitSucessfull: false,
                                    error:
                                        error_json_body.message ||
                                        error.message,
                                });
                                if (error_json_body) {
                            ...
                            .then(() => {
                                ...
                                this.props.configUpdatedHandler(
                                    formattedConfig
                                );
                                resetForm();
                            })
                ...
                {this.state.submitSucessfull !== null ? (
                    this.state.submitSucessfull ? (
                        <Alert variant="success" style={{marginTop: "16px"}}>
                            Configuration sucessfully saved
                        </Alert>
                    ) : (
                        <Alert variant="danger" style={{marginTop: "16px"}}>
                            {this.state.error
                                ? this.state.error
                                : "There has been an error while submitting the form. Please retry."}
                            <br />
                        </Alert>
                    )
                ) : (
